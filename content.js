(() => {
  ...
  function createOverlay(video, sources) {
    ...
    if ((hasBlobOnly || hasNoSources) && isVideoSite) {
      ...
      // Also add a "Try with Motrix" button for the page URL
      const tryItem = document.createElement('div');
      tryItem.className = 'motrix-source-item motrix-source-primary';
      tryItem.innerHTML = `
        <span class="motrix-source-label">⬇ Send Page URL to Motrix</span>
        <span class="motrix-source-action">Download</span>
      `;
      tryItem.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        handleDownload(pageUrl, panel);
      });
      sourceList.appendChild(tryItem);
    }
